...
export default class Index extends React.Component {
  ...
  // 获取轮播图数据的方法
  async getSwipers() {
    const res = await axios.get('http://incognito.tpddns.cn:8081/home/swiper')
    this.setState({
  ...
  // 获取租房小组数据的方法
  async getGroups() {
    const res = await axios.get('http://incognito.tpddns.cn:8081/home/groups', {
      params: {
  ...
  async componentDidMount() {
    ...
    //     `http://incognito.tpddns.cn:8081/area/info?name=${res.name}`
    //   )
    //   // console.log(result)
    //   this.setState({
          ...
          <Grid
            ...
            renderItem={item => (
              <Flex className="group-item" justify="around" key={item.id}>
                ...
                <img src={`http://incognito.tpddns.cn:8081${item.imgSrc}`} alt="" />
              </Flex>
            )}
          />
        </div>
